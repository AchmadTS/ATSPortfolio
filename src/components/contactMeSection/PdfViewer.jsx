...
const PdfViewer = ({ show, currentPdf, onClose }) => {
  ...
  return (
    <div
      className={`fixed inset-0 bg-black/90 flex items-center justify-center z-[60] p-2 sm:p-4 transition-opacity duration-300 ${isPdfOpening ? "opacity-100" : "opacity-0"}`}
    >
      <div
        className={`bg-gray-900 rounded-xl w-full max-w-5xl h-[95vh] sm:h-[90vh] border border-white/20 relative transition-all duration-300 ${isPdfOpening && !isPdfClosing ? "scale-100 opacity-100" : "scale-95 opacity-0"} flex flex-col`}
      >
        <div className="flex items-center justify-between p-3 sm:p-4 border-b border-white/20 gap-2">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs sm:text-sm text-gray-400 hidden sm:block truncate">
              {currentPdf.name}
            </p>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            ...
            >
              <FiDownload className="w-4 h-4 sm:w-4 sm:h-4 md:w-5 md:h-5" />
